...
import { sabahAdhkar, type AdhkarItem, type LocaleText } from '@/data/adhkar';
...
const SabahAdhkarPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] p-4">
      <header className="w-full max-w-lg mb-8 text-center">
        <h1 className="text-3xl font-bold text-primary">{t('أذكار الصباح')}</h1>
      </header>
          ...
          if (isAutoSwipeEnabled || isAutoTicking) {
            if (currentAdhkarIndex < adhkarList.length - 1) {
              ...
              toast({
                ...
                description: t("لقد أتممت جميع أذكار الصباح."),
              });
            }
          }
        }} 
      ...
      <div className="flex items-center space-x-2 rtl:space-x-reverse mt-6 justify-center">
        <CustomToggleSwitch
          id="auto-swipe-toggle-sabah"
          checked={isAutoSwipeEnabled}
          onChange={setIsAutoSwipeEnabled}
        />
        <Label htmlFor="auto-swipe-toggle-sabah" className="text-sm text-muted-foreground">
      ...
      <div className="w-full max-w-lg mx-auto mt-8">
        <h2 className="text-xl font-semibold text-center text-primary mb-3">{t('الاستماع إلى أذكار الصباح:')}</h2>
...
export default SabahAdhkarPage;
