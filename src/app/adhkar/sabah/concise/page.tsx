...
import { sabahAdhkarConcise, type AdhkarItem, type LocaleText } from '@/data/adhkar';
...
const SabahAdhkarConcisePage: React.FC = () => {
  ...
  useEffect(() => {
    setAdhkarList(sabahAdhkarConcise);
  }, []);
  ...
  if (adhkarList.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-150px)] p-4 text-xl">
        {sabahAdhkarConcise.length === 0 ? t("لا توجد أذكار صباح مختصرة حالياً.") : t("جاري تحميل الأذكار...")}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] p-4">
      <header className="w-full max-w-lg mb-8 text-center">
        <h1 className="text-3xl font-bold text-primary">{t('أذكار الصباح المختصرة')}</h1>
      </header>
          ...
          if (isAutoSwipeEnabled || isAutoTicking) {
            if (currentAdhkarIndex < adhkarList.length - 1) {
              ...
              toast({
                ...
                description: t("لقد أتممت جميع أذكار الصباح المختصرة."),
              });
            }
          }
        }} 
      ...
      <div className="flex items-center space-x-2 rtl:space-x-reverse mt-6 justify-center">
        <CustomToggleSwitch
          id="auto-swipe-toggle-sabah-concise"
          checked={isAutoSwipeEnabled}
          onChange={setIsAutoSwipeEnabled}
        />
        <Label htmlFor="auto-swipe-toggle-sabah-concise" className="text-sm text-muted-foreground">
...
export default SabahAdhkarConcisePage;
